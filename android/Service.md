...
- [Service](#service)
    - [1. 概述](#1-概述)
    - [2. Service在清单文件中的声明](#2-service在清单文件中的声明)
    - [3. Service启动服务](#3-service启动服务)
        - [3.1 继承Service](#31-继承service)
        - [3.2 继承IntentService](#32-继承intentservice)
    - [4. Service绑定服务](#4-service绑定服务)
        - [4.1 扩展`Binder`类 绑定服务](#41-扩展binder类-绑定服务)
            - [4.1.1 扩展 Binder 类](#411-扩展-binder-类)
            - [4.1.2 实现 ServiceConnection接口](#412-实现-serviceconnection接口)
            - [4.1.3 调用 bindService()，传递 ServiceConnection 对象](#413-调用-bindservice传递-serviceconnection-对象)
            - [4.1.4 调用 unbindService()断开与服务的连接。](#414-调用-unbindservice断开与服务的连接)
        - [4.2 绑定服务的生命周期](#42-绑定服务的生命周期)
        - [4.3 绑定时机](#43-绑定时机)
    - [5. 前台Service](#5-前台service)
    - [6. 问题：](#6-问题)
        - [6.1 service和activity](#61-service和activity)
        - [6.2 service和Thread](#62-service和thread)
